...
EOF
  ...
  configName=$(jq -r .[0].object.metadata.name $BINDING_CONTEXT_PATH)
	...
	deployment=$(kubectl get deployments.apps -o json | jq -r '.items[]|[{"name": .metadata.name, "volumes":
	.spec.template.spec.volumes[]|select(.|has("configMap"))|.configMap.name|select(. == "'${configName}'")}]|.[].name'|  tr "\n" " ")
